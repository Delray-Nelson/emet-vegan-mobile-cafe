...
export default function Checkout() {
  ...
  const estimatedDeliveryInfo = useMemo(() => {
    const now = new Date();
    // 60-min prep + 15-25 min delivery drive transit based on ZIP
    const transitMins = (zip === "30281" || zip === "30228") ? 25 : 18;
    const totalMinutes = 60 + transitMins;
    const startEta = new Date(now.getTime() + totalMinutes * 60 * 1000);
    const endEta = new Date(now.getTime() + (totalMinutes + 15) * 60 * 1000);

    const fmtTime = (d) => d.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" });
    const timeRange = `${fmtTime(startEta)} – ${fmtTime(endEta)}`;

    return {
      windowId: startEta.toISOString(),
      label: `Today between ${timeRange}`,
      timeRange,
      prepTime: "60 mins fresh kitchen prep",
      transitTime: `~${transitMins} mins courier transit to ${zip || "your address"}`
    };
  }, [zip]);
      ...
      <div className="co-grid">
        <section className="co-main">
          ...
          <div className="co-block e-card">
            ...
            {/* Auto-Calculated Delivery Timeline banner inside Address card */}
            <div className="co-auto-eta-panel">
              <div className="co-auto-eta-header">
                <span className="co-auto-eta-title">⚡ Auto-Calculated Delivery Arrival</span>
                <strong className="co-auto-eta-time">{estimatedDeliveryInfo.timeRange}</strong>
              </div>
              <div className="co-auto-eta-steps">
                <div className="co-eta-step">
                  <span className="co-eta-dot">🍳</span>
                  <span><strong>60 min</strong> Fresh kitchen prep</span>
                </div>
                <div className="co-eta-step">
                  <span className="co-eta-dot">🚗</span>
                  <span><strong>{estimatedDeliveryInfo.transitTime}</strong></span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Contact Details */}
          <div className="co-block e-card">
            <h2 className="co-h2">2. Your Contact Details</h2>
            <label className="co-field">
              <span>Full Name *</span>
              <input
                value={name}
        ...
        <aside className="co-summary e-card">
          ...
          <div className="co-eta-badge">
            <span className="co-eta-icon">🚗</span>
            <div>
              <span className="co-eta-title">Estimated Delivery Window</span>
              <strong className="co-eta-time">{selectedWindowLabel}</strong>
            </div>
          </div>
